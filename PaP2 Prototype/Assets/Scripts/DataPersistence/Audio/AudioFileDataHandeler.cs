...
public class AudioFileDataHandeler
{
    ...
    public AudioFileDataHandeler(string dataDirPath, string dataFileName, bool useEncryption)
    {
    ...
    public AudioData Load()
    {
        ...
        if (File.Exists(fullPath))
        {
            try
            {
                ...
                LoadedData = JsonUtility.FromJson<AudioData>(dataToLoad);
    ...
    public void Save(AudioData data)
    {
        //Path.Combine allows any system to use the file path. MAC, Windows, etc. all have their own set up for file paths
        string fullPath = Path.Combine(dataDirPath, dataFileName);
        try
